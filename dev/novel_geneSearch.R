...
# setwd("Z:/resources/GeneSearch/genome/gemini_tsv_filtered")
# args <- c("E10028KDS5.tsv.gz", "ABCA4.clinvar.hgmd.OGLanno.select.xlsx", "crossmap.hg19.gene.hgmd.clinvar__chr1.tsv", "test.gene.hgmd.clinvar__chr1.ps.tsv")
...
OGLanno <- read_tsv(Input_file, col_names = TRUE, na = c("NA", "", "None", "NONE", ".", "FALSE", "False"), col_types = cols(.default = col_character())) %>%
  ...
  select('gene', 'sample', 'chr_variant_id', 'grch37variant_id', 'gts', 'gt_types',
         ...
         'insilico_score', 'gno2e3g_af', 'gno2e3g_acan', 'pmaxaf', 'gno2x_af_all',
...
#openxlsx::write.xlsx(list("OGLanno" = highImpact_gene), file = "test.xlsx", firstRow = TRUE, firstCol = TRUE)
